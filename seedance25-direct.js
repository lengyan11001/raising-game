...
function validateSeedance25DirectInput(input = {}) {
  ...
  if (!SEEDANCE25_DIRECT_RESOLUTIONS.includes(resolution)) {
    throw directInputError("SEEDANCE25_DIRECT_INVALID_RESOLUTION", "Seedance 2.5 resolution must be 480p or 720p.", { allowed: SEEDANCE25_DIRECT_RESOLUTIONS });
  }
  if (!SEEDANCE25_DIRECT_RATIOS.includes(ratio)) {
    throw directInputError("SEEDANCE25_DIRECT_INVALID_RATIO", "Seedance 2.5 ratio is not supported.", { allowed: SEEDANCE25_DIRECT_RATIOS });
  }
  ...
  if (!Number.isInteger(duration) || duration < 4 || duration > 30) {
    throw directInputError("SEEDANCE25_DIRECT_INVALID_DURATION", "Seedance 2.5 duration must be an integer from 4 to 30 seconds.", { min: 4, max: 30 });
  }

  if (mode === "omini") {
    if (images.length > 30) throw directInputError("SEEDANCE25_DIRECT_TOO_MANY_IMAGES", "Seedance 2.5 supports at most 30 reference images.");
    if (videos.length > 10) throw directInputError("SEEDANCE25_DIRECT_TOO_MANY_VIDEOS", "Seedance 2.5 supports at most 10 reference videos.");
    if (audios.length > 10) throw directInputError("SEEDANCE25_DIRECT_TOO_MANY_AUDIOS", "Seedance 2.5 supports at most 10 reference audios.");
    if (images.length + videos.length + audios.length > 50) throw directInputError("SEEDANCE25_DIRECT_TOO_MANY_ASSETS", "Seedance 2.5 supports at most 50 reference assets in total.");
